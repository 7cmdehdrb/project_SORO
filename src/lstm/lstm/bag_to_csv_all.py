...
def dump_bag(bag_path):
    ...
    while reader.has_next():
        ...
        if hasattr(msg, "header"):
            if init_t is None:
                init_t = msg.header.stamp.sec + 1e-9 * msg.header.stamp.nanosec

            t = msg.header.stamp.sec + 1e-9 * msg.header.stamp.nanosec - init_t
        else:
            if init_t is None:
                init_t = ts * 1e-9
            t = ts * 1e-9 - init_t
...
if len(sys.argv) == 2:
    ...
    for folder in folders:
        inner_file_num = len(os.listdir(os.path.join(sys.argv[1], folder)))
        if inner_file_num != 2:
            continue
        print(f"Processing folder: {folder}")
        dump_bag(os.path.join(sys.argv[1], folder))
